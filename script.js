...
const players = Array.from({ length: 100 }, (_, i) => ({
  name: `Player ${i + 1}`,
  position: ["QB", "WR", "RB", "OT", "CB", "S", "EDGE", "DL", "OL"][i % 9],
  rank: i + 1
}));

let draftOrder = [];
for (let round = 1; round <= 3; round++) {
  draftOrder.push(...teams.map((team, i) => ({ round, pick: i + 1, team })));
}
...
function pickNext(index) {
  ...
  if (!userHasPick && currentPick.pick === 1) {
  ...
  if (currentPick.team === userTeam) {
    playerChoices.innerHTML = "<h3>Your Turn! Select a Player:</h3>";
    ...
    addPickToBoard(currentPick, bestPlayer, false);
    pickNext(index + 1);
  }
}
...
function simulateRound(round) {
  ...
  picks.forEach(p => {
    const bestPlayer = availablePlayers.shift();
    addPickToBoard(p, bestPlayer, false);
  });
}
...
function addPickToBoard(pick, player, isUser) {
  ...
  if (isUser) row.style.fontWeight = "bold";
